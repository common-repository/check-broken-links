...
class wblm_List_Table extends WP_List_Table {
	...
	function usort_reorder( $a, $b ) {
		...
		$orderby = ( ! empty( $_GET['orderby'] ) ) ? sanitize_sql_orderby($_GET['orderby']) : 'old_url';
		...
		return '
			<div class="blc-inline-editor-content">
				<label>
					...
					<span><input type="text" placeholder="http://" name="new_url%s" value="" class="wcbl-table-add-field" /></span>
				</label>
	...
	function column_old_url($item){
		...
		$actions = array(
			...
			'log'    => sprintf('<a href="'. admin_url("admin.php?page=%s&url=%s") .'">Log</a>','wcbl-log',$item['id']), 
		...
		if( 'delete'===$this->current_action() ) {
			foreach($_POST['url'] as $url) {
				if(is_numeric($url) && $url!=null){	
					$wpdb->delete(TABLE_WBLM, array( 'id' => $url ), array( '%d' ));
					$wpdb->delete(TABLE_WBLM_LOG, array( 'url' => $url ), array( '%d' ));
				}	
			}
			...
			get_bulkEdit('wcbl-broken', 'Add');
		}
		
		$urlAdd  = isset($_REQUEST['urlAdd']) ? $_REQUEST['urlAdd'] : null;
		if($urlAdd){
			if(is_numeric($urlAdd) && $urlAdd!=null){
				$new_url  = isset($_REQUEST['new_url'.$urlAdd]) ? sanitize_url($_REQUEST['new_url'.$urlAdd]) : null;			
				$wpdb->query($wpdb->prepare("UPDATE " . TABLE_WBLM . " SET `new_url` = '%s', `active` = '1' WHERE id = '%d'", $new_url, $urlAdd));
				
			}
		}	
		$urlAddBulk  = isset($_REQUEST['urlAddBulk']) ? 'OK' : null;
		if($urlAddBulk){
			$new_url  = isset($_REQUEST['new_url']) ? sanitize_url($_REQUEST['new_url']) : null;
	...
	function prepare_items($search=''){
		...
		$orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? sanitize_sql_orderby($_REQUEST['orderby']) : 'hit';
